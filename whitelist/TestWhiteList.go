...
func main() {
	...
	if err == nil {
		...
		if dReceipt != nil {
			...
			log.Printf("trustee 1:%v", isAcc1Trustee)
			//print trustees for account2
			isAcc2Trustee, _ := contract.TrueTrustee(&bind.CallOpts{}, common.HexToAddress(account2))
			log.Printf("trustee 2:%v", isAcc2Trustee)
			//add account 2 to trustee
			isAcc2Trustee = addTrustee(conn, contract, account2, auth1)
			log.Printf("trustee 2:%v", isAcc2Trustee)
...
func addTrustee(client *ethclient.Client, contract *whitelist.Administration, admin string, sender *bind.TransactOpts) bool {
	result := false
	addTrusteeTx, err := contract.AddTrustee(sender, common.HexToAddress(admin))
	if err == nil {
		uReceipt, _ := u.MineTransaction(copyClient(client), addTrusteeTx)
		if uReceipt != nil {
			//get voting contract
			actualVoting, _ := contract.Actual(&bind.CallOpts{})
			votingAddress := actualVoting.Voting
			log.Printf("votingAddress:%v", votingAddress.Hex())
			votingContract, _ := whitelist.NewVoting(votingAddress, client)
			//accept voting
			acceptTx, err := votingContract.Accept(copyAuth(sender))
			if err != nil {
				log.Printf("Failed to accept voting : %v\n", err)
			}
			aReceipt, _ := u.MineTransaction(copyClient(client), acceptTx)
			if aReceipt != nil {
				//count result
				countTx, err := votingContract.CountResult(copyAuth(sender))
				if err != nil {
					log.Printf("Failed to count voting : %v\n", err)
				}
				cReceipt, _ := u.MineTransaction(copyClient(client), countTx)
				if cReceipt != nil {
					result = true
				}
			}
